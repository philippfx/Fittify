...
namespace Fittify.Api.Controllers.Sport
{
        ...
        Controller
    {
            ...
            IHttpContextAccessor httpContextAccesor)
        {
            ...
            _hateoasLinkFactory = new HateoasLinkFactory<int>(_urlHelper, nameof(WorkoutApiController));
        ...
        [RequestHeaderMatchesApiVersion(new[] { "1" })]
        public async Task<IActionResult> GetCollection(WorkoutOfmCollectionResourceParameters collectionResourceParameters)
        {
            var stringGuid = User.Claims.FirstOrDefault(c => c.Type == "sub")?.Value;
            if (String.IsNullOrWhiteSpace(stringGuid)) return Unauthorized();
            var ownerGuid = new Guid(stringGuid);
            ...
            if (_incomingHeaders.IncludeHateoas) expandableOfmForGetCollection = expandableOfmForGetCollection.CreateHateoasForExpandableOfmForGets<WorkoutOfmForGet, int>(_urlHelper, nameof(WorkoutApiController), collectionResourceParameters.Fields).ToList(); // Todo Improve! The data is only superficially shaped AFTER a full query was run against the database
            ...
            this.AddPaginationMetadata<int, WorkoutOfmForGet>(ofmForGetCollectionQueryResult,
                _incomingHeaders, collectionResourceParameters.AsDictionary().RemoveNullValues(), _urlHelper, nameof(WorkoutApiController));
        ...
        public async Task<IActionResult> UpdatePartially(int id, [FromBody]JsonPatchDocument<WorkoutOfmForPatch> jsonPatchDocument)
        {
            ...
            var ofmForPatch = await _asyncOfmRepository.GetByIdOfmForPatch<WorkoutOfmForPatch>(id);
            if (ofmForPatch == null)
            {
